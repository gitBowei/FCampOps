...
//$Powershellpath = "https://ps.compliance.protection.outlook.com/powershell-liveid/"
$Powershellpath = "https://ps.outlook.com/powershell-liveid/"
try{
if ($Credentials) {
$Session = New-PSSession -ConfigurationName Microsoft.Exchange -ConnectionUri $Powershellpath -Credential $Credentials -Authentication Basic -AllowRedirection -ErrorAction Stop
}
else {
...
//Get-MessageTrace -StartDate (Get-Date).Adddays(-3) -EndDate (get-date) -RecipientAddress "*@campohenriquez.com"
//Get-MessageTrace -StartDate (Get-Date).AddHours(-30) -EndDate (get-date) -RecipientAddress "*@campohenriquez.com"
//$DateEnd = Get-Date
...
//| Select-Object MessageTraceId,MessageID,SenderAddress,RecipientAddress,Subject,Received,Status,FromIP,ToIP,Size
