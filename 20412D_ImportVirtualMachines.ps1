...
$path = $drive + ":\Program Files\Microsoft Learning\20412\Drives\"

Import-VM -Path (Get-Item ($path + "20412D-LON-CL1\Virtual Machines\*.xml"))
Import-VM -Path (Get-Item ($path + "20412D-LON-CL2\Virtual Machines\*.xml"))
Import-VM -Path (Get-Item ($path + "20412D-LON-CA1\Virtual Machines\*.xml"))
Import-VM -Path (Get-Item ($path + "20412D-LON-CORE\Virtual Machines\*.xml"))
Import-VM -Path (Get-Item ($path + "20412D-LON-DC1\Virtual Machines\*.xml"))
Import-VM -Path (Get-Item ($path + "20412D-LON-SVR1\Virtual Machines\*.xml"))
Import-VM -Path (Get-Item ($path + "20412D-LON-SVR2\Virtual Machines\*.xml"))
Import-VM -Path (Get-Item ($path + "20412D-LON-SVR3\Virtual Machines\*.xml"))
Import-VM -Path (Get-Item ($path + "20412D-LON-SVR4\Virtual Machines\*.xml"))
Import-VM -Path (Get-Item ($path + "20412D-TOR-DC1\Virtual Machines\*.xml"))
Import-VM -Path (Get-Item ($path + "20412D-TREY-CL1\Virtual Machines\*.xml"))
Import-VM -Path (Get-Item ($path + "20412D-TREY-DC1\Virtual Machines\*.xml"))
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUrAFA/rS2CxTTYPUJKXC6Fud8
# BgOgghWCMIIEwzCCA6ugAwIBAgITMwAAAEyh6E3MtHR7OwAAAAAATDANBgkqhkiG
...
# HAYKKwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFKFo
# XX5OQ8bPKPkpz5t21IGYQgllMF4GCisGAQQBgjcCAQwxUDBOoCaAJABNAGkAYwBy
# AG8AcwBvAGYAdAAgAEwAZQBhAHIAbgBpAG4AZ6EkgCJodHRwOi8vd3d3Lm1pY3Jv
# c29mdC5jb20vbGVhcm5pbmcgMA0GCSqGSIb3DQEBAQUABIIBAKmYXvVkkfc5+hA9
# pMtQXvLhYMQkzYF/IOVvYDpFNZOfgtYjQcdkPxnOk73s+QHO1KP4t2atF2s/AkLr
# yX2nNDeVs+Kxflw1NNjem1PYyiKd3kGnyefg8mmv4iih/bBW9hk7yerS/nBMP0Jc
# PtmndXTpsicsDEshPQNygvv0rb19k/muxlIqUG35zGw6ViW4JxqmZsQfZX5DnGpT
# z4aLxemB0gV4bVob0fUbzpYI7OXQ/3vIHINVPnP3M4lcgIg3Ou7JfV/qO3Pm9Qpr
# XCta2SlW94wKcKESN+Qfwbpae9w5cpektjMe58nx/zu2KLlUlWa6pD1fVvlkcQTC
# zmRCuI+hggIoMIICJAYJKoZIhvcNAQkGMYICFTCCAhECAQEwgY4wdzELMAkGA1UE
...
# MjcwNzQzNDdaMCMGCSqGSIb3DQEJBDEWBBQJeoJ2mwDdpVN9Q6X5MaTMZ4peWTAN
# BgkqhkiG9w0BAQUFAASCAQAz6CCiaR51b7uQmEPAJLRdW0Joh1tLvdVcOEhlpCi8
# udUL7Me5l/1ygpQrYhBZ5FTZceYUtnxMcbo1MiA0HGeaHy0N4mP5IbBwoabS3T+E
# n4mLK+47gmTSnQR8jK/PkMWq8kao/oSUgMWmpRIhi76ehpQFfw+ZqREyDC00z3Rn
# LANeyuhyFClIpvEMBeaMMZpBnalbUfBMB1e/xj6YoRwannEHln+perL+kiP5bSuK
# 1zR0DES1aNZ1ly6GSR6CbF6vvZw9c6jc6S5eosQxrV0N1J0Tzh1FXNpluiohwoIh
# DF5OczSEY1zZYnk2cgpXuFhc/vl/XYNJYWTbCqLIXf8h
